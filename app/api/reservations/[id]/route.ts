...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const res = await db.select().from(reservations)
    .where(and(eq(reservations.id, params.id), eq(reservations.restaurantId, session.restaurantId))).get()
  ...
  const updated = await db.select().from(reservations).where(eq(reservations.id, params.id)).get()
