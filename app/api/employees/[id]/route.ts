...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const emp = await db.select().from(employees)
    .where(and(eq(employees.id, params.id), eq(employees.restaurantId, session.restaurantId))).get()
  ...
  const updated = await db.select().from(employees).where(eq(employees.id, params.id)).get()
