  ...
  if (!restaurant) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <p className="text-gray-700 font-medium">מסעדה לא נמצאה</p>
      </div>
    </div>
  )
              ...
              <>
                ...
                {categories.filter(c => c.id === activeCategory || !activeCategory).map(cat => (
                  <div key={cat.id} className="mb-8">
                    {!activeCategory && <h2 className="text-lg font-bold text-gray-900 mb-4">{cat.name}</h2>}
          ...
          <div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <textarea value={reservationForm.notes} onChange={e => setReservationForm(f => ({ ...f, notes: e.target.value }))} placeholder="בקשות מיוחדות, יום הולדת, כיסא לתינוק..." className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" rows={2} />
              </div>
              <Button className="w-full" size="lg" onClick={submitReservation} loading={submitting} disabled={!reservationForm.customerName || !reservationForm.date}>
                שלח בקשת הזמנה
              </Button>
            </div>
          </div>
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map(r => (
                    <button key={r} onClick={() => setFeedbackForm(f => ({ ...f, rating: r }))}
                      className={`text-3xl transition-transform hover:scale-110 ${r <= feedbackForm.rating ? 'text-yellow-400' : 'text-gray-200'}`}>
                      ★
                    </button>
                  ))}
                </div>
